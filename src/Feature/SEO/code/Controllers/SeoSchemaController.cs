                        ...
                        foreach (var schemaName in getSiteConfiguration.SeoSettings.SelectSeoSchema)
                        {
                            switch (schemaName)
                            {
                                ...
                                case Constants.SeoSchema.ProductSchema:
                                    ...
                                    {
                                        var productVariants = HelperExtension.GetProductVariants(HelperExtension.GetFirstMatchingChildItem(Sitecore.Context.Item, Template.Templates.VariantGroupingTemplateId));
                                    ...
                                    break;
                                case Constants.SeoSchema.ArticleSchema:
                                    if(Context.Item.TemplateID.ToString() == Template.Templates.ArticlePageTemplate)
                                    {
                                        ScriptBuilder(stringBuilder, seoSchemaGenerator.CreateArticleStructuredData(jsonsettings, getSiteConfiguration.BrandSettings.BrandLogo, getSiteConfiguration.BrandSettings.BrandName));
                                    }
                                    break;
                                default:
                                    break;
