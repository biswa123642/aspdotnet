...
Sitecore.InitBucketList = function (id, clientId, pageNumber, searchHandlerUrl, filter, databaseUrlParameter, typeToSearchString, of, enableSetStartLocation) {
    ...
    self.GetScLanguage = function () {

        var current = window;
        var top = window.top;
        var language = document.getElementById("scLanguage");

        while (language == null && current != top) {
            current = current.parent;
            language = current.document.getElementById("scLanguage");
        }

        return language && language.value;
    };

    self.contentLanguage = self.GetScLanguage();
    ...
    // Constructor for request success handler
    self.SuccessHandler = function (multilist) {
        return function (request) {
            ...
            for (i = 0; i < reducedItems.length; i++) {
                item = reducedItems[i];
                let segmentList = (item.Path).split('/').reverse();
                let segmentPath = (segmentList.slice(segmentList, 3).reverse()).join('/');
                multilist.options[multilist.options.length] = new Option(segmentPath, item.ItemId);
            }
