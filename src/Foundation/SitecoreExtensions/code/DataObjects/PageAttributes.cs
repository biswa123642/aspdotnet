...
namespace CGP.Foundation.SitecoreExtensions.DataObjects
{
    public class PageAttributes : IEnumerable<PageAttribute>
    {
        ...
        /// Loads Data 
        /// </summary>
        public void InitPageAttribute(Item page)
        {
            var pageitem = new ProductBaseItem(page);
            if (pageitem.PageAttributeField == null)
                return;
            using (new Sitecore.Globalization.LanguageSwitcher(page.Language))
            {


                var values = pageitem.PageAttributeField.GetItems();
        ...
        public void InitProductAttribute(Item page)
        {
            var pageitem = new ProductBaseItem(page);
            if (pageitem.ProductAttributeField == null)
                return;
            using (new Sitecore.Globalization.LanguageSwitcher(page.Language))
            {
                var values = pageitem.ProductAttributeField.GetItems();
